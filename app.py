...
def process_queue():
    ...
    if queue:
        ...
        if process_order(order):
            ...
            logger.info(f"Queue size after successful processing: {len(queue)}")
        else:
            logger.warning(f"Failed to process order {order.get('order_number', 'Unknown')}, returning to queue")
